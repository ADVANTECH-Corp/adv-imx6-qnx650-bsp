...
**
** Ident: $Id: flash_probe.c 658671 2012-07-09 15:30:24Z builder@qnx.com $
**
*/
...
f3s_flashcfg_t  flashcfg = {0,0,0,0};
		...
		f3s_dbase_t * dbase)
{
	...
	{
		/* workaround for Spansion lock feature */
		if ((*flash_ptr_v2)->v2lock == f3s_aCFI_v2plock) {
			printf("persistent lock is selected \n");
			dbase->flags |= F3S_PROTECT_PERSISTENT;
		}
		if ((*flash_ptr_v2)->v2lock == f3s_aCFI_v2dlock) {
			printf("dynamic lock is selected \n");
			dbase->flags |= F3S_PROTECT_DYN;
		}
...
__SRCVERSION( "$URL: http://svn/product/tags/public/bsp/nto650/freescale-mx6q-sabreARD/latest/src/hardware/flash/mtd-flash/flash_probe.c $ $Rev: 658671 $" );
