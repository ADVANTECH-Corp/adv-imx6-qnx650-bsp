...
create_device(TTYINIT_MX1 *dip, unsigned unit)
{
    ...
    if(dip->usedma && MX53_UART3_BASE_ADDR != dip->tty.port)
    {
        fprintf(stderr, "DMA is only supported for UART3 on for MX53\n");
        dip->usedma = 0;
    }
    ...
    if(dip->usedma)
        dev->tty.highwater = dev->tty.ibuf.size - DMA_XFER_SIZE;
    else
    ...
    if(dev->usedma)
    {
        ...
        sprintf(str, "eventnum=%ld,watermark=%d,fifopaddr=0x%x", dev->rx_dma_evt, ((dev->fifo & 0x3f)-1) , dip->tty.port + 0x0);
        ...
        sprintf(str, "eventnum=%ld,watermark=%d,fifopaddr=0x%x\n", dev->tx_dma_evt, (32 - ((dev->fifo >> 10) & 0x3f))  , dip->tty.port+MX1_UART_TXDATA);
    ...
    */
    ...
    /* Assert CTS (on this hardware CTS is the output and RTS is the input) */
    dev->cr2 = in32(dev->base + MX1_UART_CR2);
    dev->cr2 |= (MX1_UCR2_IRTS | MX1_UCR2_CTS);    /* Ingore RTS input for now, let ser_stty() deal with that */
    dev->cr2 &= ~(MX1_UCR2_CTSC);    /* Disable auto input hardware flow */
    out32(dev->base + MX1_UART_CR2, dev->cr2 | MX1_UCR2_SRST);
...
__SRCVERSION( "$URL: http://svn/product/tags/public/bsp/nto650/freescale-mx6q-sabrelite/latest/src/hardware/devc/sermx1/init.c $ $Rev: 633202 $" );
