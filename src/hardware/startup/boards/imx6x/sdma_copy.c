 ...
 * Note that the memory to memory SDMA ROM script has a max transfer size of (64KB-1K) 
 */
 ...
 * however for performance reasons SDMA copy size must be at least 4 byte aligned.
 */
#define MAX_SDMA_XFER_SIZE              ((16 * 1024) - 1024)
...
int sdma_init(void)
{
	...
	cmd_bd_ptr[0].cmd_and_status |= SDMA_RAM_CODE_SIZE / 2;
...
static int sdma_xfer(int ch_num, uint32_t src, uint32_t dst, int len)
{
	...
	length_low = (uint32_t)len & 0xffff;
	length_high = (((uint32_t)len >> 16) & 0x7f) << 24;
 ...
 */
void sdma_copy(uint32_t dst, uint32_t src, int size)
{
	int		dmasz;
	...
	while (size > 0) {
		dmasz = size >= MAX_SDMA_XFER_SIZE ? MAX_SDMA_XFER_SIZE : size;
...
void sdma_load_ifs(uint32_t dst, uint32_t src, int size)
{
	int		dmasz;
	...
	while (size > 0) {
		...
		if (lsp.mdriver.size > 0)
			dmasz = dmasz >= mdriver_max ? mdriver_max : size;

		// Regardless of mdriver_max the maximum SDMA copy size is still 16M-1 when using SDMA AP_2_AP
		// RAM script.
		dmasz = size >= MAX_SDMA_XFER_SIZE ? MAX_SDMA_XFER_SIZE : size;

		// Transfer data then loop
		sdma_xfer(SDMA_DATA_CH, src, dst, dmasz);
		size -= dmasz;
		dst += dmasz;
		src += dmasz;
