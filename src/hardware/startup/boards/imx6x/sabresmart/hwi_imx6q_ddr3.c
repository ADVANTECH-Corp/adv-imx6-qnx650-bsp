...
void hwi_imx6dq()
{
	...
	/* add  UART */
	{
		...
		/* create uart0 */
		HWIATTR_UART_SET_LOCATION(&attr, MX6X_UART1_BASE, MX6X_UART_SIZE, 0, hwi_find_as(MX6X_UART1_BASE, 1));
		hwi_off = hwidev_add_uart(IMX6_HWI_UART, &attr, hwi_bus_internal);
		ASSERT(hwi_find_unit(hwi_off) == 0);
		hwitag_set_ivec(hwi_off, 0, MX6X_UART1_IRQ);
	...
	{	
		...
		hwiattr_common_t attr = HWIATTR_COMMON_INITIALIZER;
		HWIATTR_SET_LOCATION(&attr, MX6X_IPU1_BASE, MX6X_IPU_SIZE, 0, hwi_find_as(MX6X_IPU2_BASE, 1));
		hwitag_add_common(hwi_off, &attr);
		ASSERT(hwi_off != HWI_NULL_OFF);
	}
