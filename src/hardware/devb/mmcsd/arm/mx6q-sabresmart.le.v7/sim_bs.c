...
int bs_init(SIM_HBA *hba)
{
	...
	// Ports 2,3 have CD and WP functionality
	if(cfg->IOPort_Base[0] == MX6X_USDHC2_BASE) {
		// port 2 uses GPIO2_2 as card detect and GPIO2_3 as write protect
		cpin = 2;
		ppin = 3;
	...
	else if (cfg->IOPort_Base[0] == MX6X_USDHC3_BASE) {
		// port 3 uses GPIO2_0 as card detect and GPIO2_1 as write protect
		cpin = 0;
		ppin = 1;
		if ((gpio_base = mmap_device_io(0x100, MX6X_GPIO2_BASE))
			== (uintptr_t)MAP_FAILED) {
