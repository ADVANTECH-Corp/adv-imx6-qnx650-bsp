...
dd if=image/u-boot_crc.bin.crc of=$1 bs=512 seek=2 conv=sync &>/dev/null
dd if=image/u-boot_crc.bin     of=$1 bs=512 seek=3 conv=sync &>/dev/null

echo "[Copy uImage]"
dd if=image/ifs-mx6q-sabresmart.raw of=$1 bs=1024k seek=5 conv=sync
